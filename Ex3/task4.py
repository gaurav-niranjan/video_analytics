...
from model import MultiScaleTCN
...
def train_epoch(model, dataloader, criterion, optimizer, writer, global_step, device):
    ...
    for index, (video_feature, labels) in enumerate(dataloader):
        ...
        final_output = outputs[2] #Select the output of the merged logits
...
def evaluate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for video_features, labels in dataloader:
            ...
            final_output = final_output.reshape(-1, final_output.shape[-1])  #(B*T, C)
...
def main():
    ...
    input_dim = 2048
    learning_rate = 0.001
    log_dir = './runs/task4_downsampled_8x'
    ...
    model = MultiScaleTCN(input_dim=input_dim, num_classes=num_classes, num_layers=10, num_filters=64).to(device)
